...
const workermodel = require('./models/worker.ts');
const postmodel = require('./models/post.ts');
const feedbackmodel = require('./models/feedback.ts');
...
app.post('/resetPassword',async function(req,res){
    ...
    try{
        ...
        console.error("Reset password error:", (err as any).message);
...
app.post('/signup_user', async function(req, res) {
    ...
    bcrypt.genSalt(10, function(err, salt) {
        bcrypt.hash(password, salt, async function(err, hash) {
            ...
            res.cookie("token", token, cookieOptions(req) as any);
...
app.post('/signup_worker', profileupload.fields([
    ...
    try {
        ...
        console.error("Worker Create Error →", (error as any).message);
...
app.get('/api/hireworker',isLoggedIn , async function(req,res){
    const workerType = req.query.worker;
    let user = await usermodel.findOne({mobile:req.user.mobile});
    res.json({user, workerType});
});
...
app.get('/api/profile', isLoggedIn, async function(req, res) {
    if (req.userType === 'user') {
        ...
        return res.json({ type: 'worker', data: worker });
    } else {
...
// Catch-all redirect for non-API GET requests
app.use((req, res, next) => {
    if (req.method === 'GET' && !req.path.startsWith('/api')) {
        return res.redirect(`http://localhost:5175${req.originalUrl}`);
    }
    next();
});
...
function isLoggedIn(req, res, next) {
    ...
    if (!token) {
        return res.redirect('http://localhost:5175/login_user');
    }
